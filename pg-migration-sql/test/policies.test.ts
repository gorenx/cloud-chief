...
describe("policies", () => {
  it("parsePoliciesFromSql extracts quoted policy names", () => {
    ...
    expect(parsePoliciesFromSql(sql)).toEqual([
      { name: "owner reads", table: "wren_state" },
      { name: "owner_writes", table: "profiles" },
    ]);
  });

  it("policiesForTable filters by table", () => {
    const sql = 'create policy "owner reads" on public.wren_state for select using (true);';
    expect(policiesForTable(sql, "wren_state")).toEqual(["owner reads"]);
