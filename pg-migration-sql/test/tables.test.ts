...
describe("pg-migration-sql", () => {
  ...
  it("parseTablesFromSql uses parser for create table", () => {
    const sql = `
      create table if not exists public.wren_state (id uuid primary key);
      create table profiles (id uuid primary key);
    `;
    expect(parseTablesFromSql(sql)).toEqual(["profiles", "wren_state"]);
  });
  ...
  it("statementReferencesTable matches policy and alter via fallback", () => {
    const policy = 'create policy "owner reads" on public.wren_state for select using (true);';
    const alter = "alter table public.wren_state enable row level security;";
    expect(statementReferencesTable(policy, "wren_state")).toBe(true);
    expect(statementReferencesTable(alter, "wren_state")).toBe(true);
  ...
  it("extractTableSql keeps only statements for the target table", () => {
    const sql = `
      ...
      create policy "owner reads" on public.wren_state for select using (true);
      create table public.other (id uuid);
    `;
    const extracted = extractTableSql(sql, "wren_state");
