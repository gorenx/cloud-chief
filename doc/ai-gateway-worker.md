...
> **API 参考（端点 / 错误码 / SSE）** → [ai-gateway-worker/API.md](../ai-gateway-worker/API.md)  
> Worker 部署名：`ai-gateway-proxy` · 源码：`ai-gateway-worker/src/` · 权益：[worker-revenuecat.md](./worker-revenuecat.md)

面向**客户端 / 应用开发者**：如何用 Supabase 登录态调用 AI 推理，而不暴露 DashScope Key 或 Gateway Token。
...
└──────────────┘   可选 X-Device-Id                        └──────────┬──────────┘
                    ...
                    验 JWT → 读权益 → 扣免费配额 → 选模型 → 注入密钥    │
                                                          ...
                                                          Cloudflare AI Gateway
                                                                      │
                                                                      ▼
                                                          阿里云百炼 MaaS (DashScope)
```

**客户端只需知道 Worker 地址 + Supabase `access_token`。**  
`DASHSCOPE_API_KEY`、`CF_AIG_TOKEN`、`SUPABASE_SERVICE_ROLE_KEY` 仅存在于 Worker Secrets，不下发应用。
...
凭据可从 `supabase.md` 或环境变量 `EMAIL` / `PASSWORD` / `ANON_KEY` 读取。
...
OpenAI Responses API 兼容（百炼 `/compatible-mode/v1/responses`）。
...
详见 [`ali-llm-api/respponse-api/create-response.md`](../ali-llm-api/respponse-api/create-response.md)。
...
| 客户端鉴权 | Supabase JWT | `Authorization: Bearer {DASHSCOPE_API_KEY}` + 通常需 `cf-aig-authorization` |
| 密钥 | 不暴露 | 需持有 DashScope + Gateway Token |
| 权益/配额 | 有（Plus / Free） | 无 |
...
| 文档 | 内容 |
|------|------|
| [doc/README.md](./README.md) | Workers 总览与架构 |
| [worker-revenuecat.md](./worker-revenuecat.md) | 订阅权益 sync |
| [ai-gateway-worker/README.md](../ai-gateway-worker/README.md) | 部署、Workers Builds |
| [ali-llm-api/](../ali-llm-api/) | 百炼上游请求/响应/SSE 细节 |
