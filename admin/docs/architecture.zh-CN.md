...
| 模型目录 | `model-catalog.ts` | 本地内置模型元数据（当前以 Qwen 为主） |
...
浏览器 ──(无 token)──────────► /config, /api/chat
                                    │
                                    └──当前 provider key, CF_AIG_TOKEN──► AI Gateway
...
| `DASHSCOPE_API_KEY` | 当前内置 provider adapter | `.env`；仅 `/api/chat`；名称尚未去供应商化 |
...
架构边界是 Gateway、Provider、API path 和 Model；不要求接入阿里模型。当前 `model-catalog.ts`、`DASHSCOPE_API_KEY` 以及部分兼容路径仍是 Qwen/DashScope 适配实现，后续提供商扩展应替换为 provider-neutral adapter/config，而不是把供应商判断散落到 Admin 或业务策略中。
```
