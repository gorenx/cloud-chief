...
import { fetchSupabaseFunctionsBrowse } from "@/lib/api";

export function FunctionsDirPicker({
  token,
  open,
  initialPath,
  onClose,
  ...
  const browseQ = useQuery({
    queryKey: ["supabase-functions-browse", token, browsePath],
    queryFn: async () => {
      const r = await fetchSupabaseFunctionsBrowse(token, browsePath);
  ...
  const currentLabel = data?.path || t("supabase.functionsDirPickerRoot");
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
          <div className="min-w-0">
            <p className="text-sm font-medium text-[var(--color-text)]">
              {t("supabase.functionsDirPickerTitle")}
            </p>
            <p className="truncate text-[10px] text-[var(--color-muted)]">
              {t("supabase.functionsDirCurrent", { path: currentLabel })}
            </p>
          </div>
          <button
            type="button"
            ...
            onClick={onClose}
            aria-label={t("supabase.functionsDirPickerCancel")}
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="max-h-72 overflow-y-auto p-2">
          {browseQ.isLoading && (
            <p className="px-2 py-4 text-xs text-[var(--color-muted)]">{t("supabase.functionsLoading")}</p>
          )}
            ...
            <ul className="space-y-0.5">
              {data.parent !== null && (
                <li>
                  ...
                  >
                    ...
                    <span>{t("supabase.functionsDirPickerParent")}</span>
                  </button>
                </li>
              )}
              ...
              {data.entries.length === 0 ? (
                <li className="px-2 py-4 text-xs text-[var(--color-muted)]">
                  {t("supabase.functionsDirPickerEmpty")}
                </li>
              ) : (
        ...
        <div className="flex items-center justify-between gap-2 border-t border-[var(--color-border)] px-4 py-3">
          <span className="text-[10px] text-[var(--color-muted)]">
            {data
              ? t("supabase.functionsDirPickerCount", { count: data.function_count })
              : null}
          </span>
          <div className="flex gap-2">
            <Button size="sm" variant="ghost" onClick={onClose}>
              {t("supabase.functionsDirPickerCancel")}
            </Button>
            <Button
              size="sm"
              disabled={!data}
            ...
            >
              {t("supabase.functionsDirPickerSelect")}
            </Button>
          </div>
        </div>
      </div>
