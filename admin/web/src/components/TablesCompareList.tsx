...
import type { SupabaseTableCompareRow } from "@/lib/api";
...
function PolicyChips({ policies, emptyLabel }: { policies: string[]; emptyLabel: string }) {
  if (policies.length === 0) {
    return <span className="text-[10px] text-[var(--color-muted)]">{emptyLabel}</span>;
  }
  return (
    <div className="flex flex-wrap gap-1">
      {policies.map((name) => (
        <Chip key={name} variant="default">
          <code className="mono text-[10px]">{name}</code>
        </Chip>
      ))}
    </div>
  );
...
function StatusChip({ status }: { status: SupabaseTableCompareRow["status"] }) {
  const { t } = useLocale();
...
function TableCompareDetail({
  row,
  onApply,
  applying,
  applyingTable,
}: {
  row: SupabaseTableCompareRow;
  onApply: (table: string) => void;
  applying: boolean;
  applyingTable?: string;
}) {
  const { t } = useLocale();
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <section className="space-y-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-panel-elevated)]/30 p-3">
          ...
          {row.source_files.length > 0 ? (
            ...
            <p className="text-[10px] text-[var(--color-muted)]">{t("supabase.tableNoSourceFiles")}</p>
          )}
          <div>
            <p className="text-[10px] font-medium text-[var(--color-muted)]">
              {t("supabase.localPolicies")}
            </p>
            <div className="mt-1">
              <PolicyChips policies={row.local_policies} emptyLabel={t("supabase.noPolicies")} />
            </div>
          </div>
        </section>
        ...
        <section className="space-y-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-panel-elevated)]/30 p-3">
          ...
          {row.remote && (
            <p
              className={cn(
                "text-[10px]",
                row.rls_enabled ? "text-emerald-300" : "text-amber-200",
              )}
            >
              {row.rls_enabled ? t("supabase.rlsOn") : t("supabase.rlsOff")}
              {row.policy_count != null && row.policy_count > 0
                ? ` · ${t("supabase.policyCount", { count: row.policy_count })}`
                : ""}
            </p>
          )}
          <div>
            <p className="text-[10px] font-medium text-[var(--color-muted)]">
              {t("supabase.remotePolicies")}
            </p>
            <div className="mt-1">
              <PolicyChips policies={row.remote_policies} emptyLabel={t("supabase.noPolicies")} />
            </div>
          </div>
        </section>
      </div>
      ...
      {row.status === "local_only" && (
        <ScrollSafeButton
          size="sm"
          disabled={applying}
          busy={applying && applyingTable === row.name}
          onAction={() => onApply(row.name)}
        >
          {applying && applyingTable === row.name
            ? t("supabase.migrationApplying")
            : t("supabase.applyTable")}
        </ScrollSafeButton>
      )}
    </div>
  );
...
export function TablesCompareList({
  rows,
  summary,
  onApply,
  applying,
  ...
  if (rows.length === 0) {
    return (
      <p className="text-xs text-[var(--color-muted)]">{t("supabase.noTablesCompared")}</p>
    );
  }
    ...
    <div className="overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-panel)]">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--color-border)] px-3 py-2.5">
        <div className="min-w-0">
          <p className="text-[11px] font-medium text-[var(--color-text)]">
            {t("supabase.tablesCompareTitle")}
          </p>
          <p className="text-[10px] text-[var(--color-muted)]">
            {t("supabase.tableCompareSummary", summary)}
          </p>
        </div>
        {onApplyAll && (pendingCount ?? 0) > 0 && (
          <ScrollSafeButton size="sm" disabled={applying} busy={applying} onAction={onApplyAll}>
            {applying
              ? t("supabase.migrationApplying")
              : t("supabase.applyAllTables", { count: pendingCount ?? 0 })}
          </ScrollSafeButton>
        )}
      </div>
      ...
      <div className="flex min-h-[16rem] flex-col sm:flex-row">
        <aside className="shrink-0 border-b border-[var(--color-border)] bg-[var(--color-panel-elevated)]/25 sm:w-[11rem] sm:border-b-0 sm:border-r">
          <nav
            ...
            aria-label={t("supabase.tableListAria")}
          >
            {rows.map((row) => {
              ...
              return (
                <button
                  key={row.name}
                  type="button"
                  {...navButtonScrollSafeProps(scrollRef, () => setSelected(row.name))}
                  className={cn(
        ...
        <main className="min-w-0 flex-1 p-4">
          {selectedRow ? (
            ...
            <p className="text-xs text-[var(--color-muted)]">{t("supabase.tableDetailEmpty")}</p>
          )}
        </main>
      </div>
    </div>
