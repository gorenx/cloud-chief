...
import { adminFetch, fetchGatewayContext, fetchKeys, fetchState } from "@/lib/api";
...
export function KeysPage() {
  ...
  const slugMatch = effectiveSlug && providers.some((p) => p.slug === effectiveSlug);

  const saveMut = useMutation({
    mutationFn: async () => {
      if (!slugMatch && effectiveSlug) {
        if (!confirm(`slug "${effectiveSlug}" 不在提供商列表中，BYOK 可能不生效。继续？`)) {
          throw new Error("已取消");
        }
      }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--color-muted)]">
          provider_slug 必须等于自定义提供商的 slug（请求 URL 中 custom- 后面部分）
        </p>
      </div>

      <Card>
        <label className="mb-1 block text-xs text-[var(--color-muted)]">选择网关</label>
        <Select
          value={gateway}
      ...
      <Card>
        ...
        {!gateway ? (
          ...
          <p className="text-sm text-[var(--color-muted)]">该网关暂无 BYOK 密钥</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {keysQ.data!.map((k) => (
                <tr key={k.id} className="border-b border-[var(--color-border)]/60">
                  <td className="py-2">
                    <code className="mono">{k.provider_slug}</code>
                  </td>
                  <td className="py-2">{k.alias}</td>
                  <td className="py-2">{k.default_config && <Chip variant="on">默认</Chip>}</td>
                  <td className="py-2 text-xs text-[var(--color-muted)]">{k.secret_preview}</td>
                  <td className="py-2">
                    <Button
                      variant="danger"
                      size="sm"
      ...
      <Card>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <label className="mb-1 block text-xs text-[var(--color-muted)]">provider_slug</label>
            <Select value={slugSel} onChange={(e) => setSlugSel(e.target.value)}>
              {providers.length === 0 ? (
          ...
          <div>
            <label className="mb-1 block text-xs text-[var(--color-muted)]">手动 slug（可选）</label>
            <Input value={slugManual} onChange={(e) => setSlugManual(e.target.value)} />
          </div>
          <div>
          ...
          <div>
            <label className="mb-1 block text-xs text-[var(--color-muted)]">密钥值</label>
            <Input type="password" value={secret} onChange={(e) => setSecret(e.target.value)} />
          </div>
        </div>
        {effectiveSlug && (
          <p className={`mt-2 text-xs ${slugMatch ? "text-emerald-400" : "text-amber-400"}`}>
            {slugMatch
        ...
        <Button
          className="mt-4"
          disabled={!gateway || !effectiveSlug || !secret.trim() || saveMut.isPending}
