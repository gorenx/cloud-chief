...
客户端用 **Supabase `access_token`** 调用；上游 DashScope / Gateway 密钥由 Worker 注入，不下发应用。
...
权益由 RevenueCat sync 写入 Supabase，见 [doc/worker-revenuecat.md](../doc/worker-revenuecat.md)。
...
- [README.md](./README.md) — 部署与开发
- [doc/ai-gateway-worker.md](../doc/ai-gateway-worker.md) — 调用指南（含架构与集成示例）
- [ali-llm-api/](../ali-llm-api/) — 百炼上游 API 细节
